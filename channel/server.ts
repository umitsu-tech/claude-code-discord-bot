#!/usr/bin/env bun
/**
 * Discord channel for Claude Code — discord-bot plugin fork.
 *
 ...
 * ~/.claude/channels/discord/access.json — managed by the /discord-bot:access skill.
 *
...
// The /discord-bot:access skill drops a file at approved/<senderId> when it pairs
...
async function handleInbound(msg: Message): Promise<void> {
  ...
  if (result.action === 'pair') {
    ...
    try {
      await msg.reply(
        `${lead} — run in Claude Code:\n\n/discord-bot:access pair ${result.code}`,
      )
    } catch (err) {
