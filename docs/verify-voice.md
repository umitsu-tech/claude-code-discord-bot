...
- [ ] 失敗する場合は `~/.claude/discord-bot/voice.log` を確認する（`READY_TIMEOUT_MS`＝20 秒で失敗するのは
  #73 で追跡中の既知の間欠症状。1 回失敗しても、もう一度 `/voice join` を打てば通ることが多い）
...
- 1 回目の `/voice join` が `VoiceConnectionStatus.Ready` に到達せず、20 秒で「ボイスチャンネルへの接続に
  失敗しました」になった。同じ環境で成功も複数回あり、間欠的な症状（Issue #73 で自動再試行を実装中）
